...
(* ------------------------------------------------------------------------- *)
(* Action of fstcart and sndcart on matrices.                                *)
(* ------------------------------------------------------------------------- *)
...
(*
let FSTCART_MAT_ZERO = CONJUNCT1 FSTCART_SNDCART_MAT_ZERO;;
let SNDCART_MAT_ZERO = CONJUNCT2 FSTCART_SNDCART_MAT_ZERO;;
*)
...
let MSUM_ROW = prove
 (`!s f:A->real^M^N i. msum s f$i = vsum s (\x. f x$i)`,
  REWRITE_TAC[CART_EQ_FULL; MSUM_COMPONENT; VSUM_COMPONENT_FULL]);;
...
let VECTORIZE_MSUM = prove
 (`!s f:A->real^M^N. vectorize (msum s f) = vsum s (\x. vectorize (f x))`,
  SIMP_TAC[CART_EQ; VECTORIZE_COMPONENT; DIMINDEX_FINITE_PROD;
           VSUM_COMPONENT_FULL; MSUM_COMPONENT]);;
